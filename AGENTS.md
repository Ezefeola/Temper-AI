...
|---|---|---|
| Gather functional requirements | `temper-analyst` | `prd-analyzer` + `workflow/analyst/report-formats` + `workflow/analyst/prd-template` (Phase 1) |
| Generate user stories | `temper-analyst` (Phase 2) | `spec-generator` (after PRD approval) |
| Define technical architecture | `temper-architect` | `workflow/architect/proposal-formats` + `workflow/architect/document-templates` |
...
| `workflow/analyst/report-formats` | Structured report formats for the analyst (gap reports, synthesis, etc.) |
| `workflow/analyst/prd-template` | PRD document template (11 sections) |
| `workflow/architect/proposal-formats` | Proposal and report formats for the architect |
| `workflow/architect/document-templates` | Document templates (backend-config, frontend-config, architecture-decision, etc.) |
| `workflow/jarvis/state-schema` | State file schema and delegation rules for the orchestrator |
...
## Document Responsibility — No overlap between architect and docs

The architect and docs agent both write to `Docs/` but generate **non-overlapping** documents.

| Document | Generated by | Purpose |
|---|---|---|
| `Docs/architecture-decision.md` | Architect | ADR with full reasoning, trade-offs, alternatives |
| `Docs/domain-model.md` | Architect | Entities, aggregates, events, business rules, Mermaid diagrams |
| `Docs/system-architecture.md` | Architect | Bounded contexts, component diagrams, integrations |
| `Docs/ARCHITECTURE.md` | Docs agent | Code conventions, testing strategy, deployment — links to architect docs |
| `Docs/SYSTEM.md` | Docs agent | Business overview, users, system flow — links to architect docs |
| `Docs/API.md` | Docs agent | API endpoint reference |
| `Docs/CHANGELOG.md` | Docs agent | Version history |
| `.temper/api-contracts.md` | Architect (post-backend) | Shared API contract extracted from built controllers |

**Rule:** The docs agent links to architect reference docs for domain model, system architecture,
and architecture decisions. It does NOT duplicate their content.

## API Contracts — Generated After Backend Build

When the project has both backend and frontend, the architect generates `.temper/api-contracts.md`
**after the backend is fully built**, as an optional step the user is asked about after final approval.

| Document | Generated by | Audience | Purpose |
|---|---|---|---|
| `.temper/api-contracts.md` | Architect (extracted from built backend) | Frontend agent | Shared routes, methods, DTO shapes for frontend services |

**Why after backend is built:** The contract is extracted from the actual controller code, not designed upfront.
This guarantees accuracy — whatever endpoints the backend actually implements become the contract.
Frontend cannot start without it (or explicit endpoint guidance per task).

**Flow:**
1. Backend tasks execute (Group 2)
2. Backend build completes → user gives final OK
3. JARVIS asks: "¿Generar api-contracts.md?"
4. If yes → delegate architect to extract contract from controllers → frontend starts
5. If no → frontend starts without contract (needs explicit task guidance)

**If api-contracts.md does not exist:** Frontend agent will ask for clarification before proceeding.
