...
6. If changes are needed, update and re-ask. If approved, confirm and indicate the next step is `/temper-tasks` (user stories are now generated by the analyst in Phase 2, after PRD approval)."
...
- **Always** indicate the next step (`/temper-tasks`) after approval (user stories are now generated by the analyst in Phase 2, not by a separate spec agent).
