...
### Phase execution — read state, act, update, stop
...
4. **Update `.temper/orchestrator-state.md`:**
...
5. **Report to user:**
   - If more phases remain: "Phase [X] complete. Start a new session and run `/temper-next` to continue to [next phase]."
...
4. **Verify build:** Ask the user to run `dotnet build`.
   - If succeeds: Update state file → `Build group: N+1 of M`, `Last build status: ok`, `Next action: "Execute Group N+1"`.
   - If fails: Update state file → `Status: blocked`, `Last build status: failed`, report errors.
5. **Report to user:**
   - If more groups remain: "Group [N] complete. Start a new session and run `/temper-next` to execute Group [N+1]."
   - If all groups complete: "Build complete. Start a new session and run `/temper-next` to proceed to review."
...
Skills to load for sub-agent: backend/dotnet/api + backend/architecture/clean
```
...
Skills to load for sub-agent: dotnet-csharp
